...
export default function Articles() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      const fetched = await getArticles("fashion");
      setArticles(fetched);
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-15">
        <div className="w-full flex items-center justify-center md:justify-between  md:py-10">
            <h1 className="text-[40px] tracking-[0.02em] text-center font-HelveticaBlack uppercase  ">Fashion</h1>
            <Filter />
        </div>

        <div className="w-full h-[500px] relative">
            <Image src="/asset/Fashion/Moody-Fashion.jpeg" alt="Fashion" fill className="w-full h-full object-cover object-top rounded-lg"  quality={100}/>
        </div>

        <h1 className="md:text-[25px] leading-[25px] text-[18px] font-HelveticaBold my-3 md:my-10">Get a behind-the-scenes look into the world of fashion through the Section Studio lens: meet up-and-coming designers, learn tips from style insiders and discover what’s next</h1>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleArticles.map((article, index) => (
              ...
              >
                 <div className="mt-2">
                   <h2 className=" text-[186x] font-HelveticaBold uppercase">{article.title}</h2>
