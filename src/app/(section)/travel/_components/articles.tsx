...
export default function Articles() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      const fetched = await getArticles("travel");
      setArticles(fetched);
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-15">
        <div className="w-full flex items-center justify-center md:justify-between  md:py-10">
            <h1 className="text-[40px] tracking-[0.02em] text-center font-HelveticaBlack uppercase  ">Travel</h1>
            <Filter />
        </div>

        <div className="w-full h-[500px] relative">
            <Image src="/asset/Travel/Ocean-Serenity.jpeg" alt="Travel" fill className="w-full h-full object-cover  rounded-lg"  quality={100}/>
        </div>

        <h1 className="md:text-[25px] leading-[25px] text-[18px] font-HelveticaBold my-3 md:my-10">Get a behind-the-scenes look into the world of travel through the Section Studio lens: meet up-and-coming artists, learn tips from style insiders and discover what’s next</h1>
